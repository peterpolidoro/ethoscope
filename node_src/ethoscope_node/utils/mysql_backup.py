...
class MySQLdbToSQlite(object):
    ...
    def _update_one_roi_table(self, table_name, src, dst, dump_in_csv=False):
        ...
        for c in dst_cur:
            if c[0] is None:
                logging.warning("There seem to be no data in %s, %s. Recreating it" % (
                os.path.basename(self._dst_path), table_name))
                self._replace_table(table_name, src, dst)
            else:
                last_id_in_dst = c[0]
    ...
    def _update_img_snapshot_table(self, table_name, src, dst):
        ...
        for c in dst_cur:
            ...
            if last_id_in_dst is None:
                logging.warning(
                    "There seem to be no data in %s, %s stopping here" % (os.path.basename(self._dst_path), table_name))
                return
